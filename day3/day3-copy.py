...
def parse_input(data):
    ...
    # Turn the entire input into a grid
    data_list = [list(line) for line in data.splitlines()]
    return data_list
...
def check_around_symbol(x, y, data, part2=False):
    ...
    for i in range(y - 1, y + 2):  # Should only check one above and one below
        if not i < 0 and not i >= len(data):
            # If the previous check was a number, this is True and ignore current number
            prev_digit = False
            for j in range(x - 1, x + 2):
                if not j < 0 and not j >= len(data[i]):
                    if data[i][j].isdigit() and not prev_digit:
                        surrounding_numbers.append(
                            int(find_number(j, data[i])))
                        prev_digit = True
    ...
    for y, line in enumerate(parsed_data):
        for x, char in enumerate(line):
            if re.match(pattern, char):
                ...
                # Add total of surrounding values to total value
                total_value += check_around_symbol(x, y, parsed_data)
    return total_value
